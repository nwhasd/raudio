...
#if defined(RAUDIO_STANDALONE)
    #ifndef TRACELOG
        #define TRACELOG(level, ...) log_log(level, "raudio", __LINE__, __VA_ARGS__)
    #endif
...
void InitAudioDevice(char * useBackends, const char * log, int logLevel)
{
    // 设置日志
    ...
    for (char *name = strtok(useBackends, ","); name != NULL; name = strtok(NULL, ","))
    {
        if (0 == strcmp(name, "wasapi"))
            backends[backendsCount] = ma_backend_wasapi;
        else if (0 == strcmp(name, "dsound"))
            backends[backendsCount] = ma_backend_dsound;
        else if (0 == strcmp(name, "winmm"))
            backends[backendsCount] = ma_backend_winmm;
        else
            continue;

        ++backendsCount;
        if (backendsCount >= ma_backend_null)
            break;
    }
    ...
    ma_result result = ma_context_init(backends, backendsCount, &ctxConfig, &AUDIO.System.context);
...
bool ExportWaveAsCode(Wave wave, const char *fileName)
{
    ...
    {
        TRACELOG(LOG_ERROR, "FILEIO: [%s] fileName is empty", fileName);
        return success;
    }

    if (! IsWaveValid(wave))
    {
        TRACELOG(LOG_ERROR, "FILEIO: [%s] wave is not ready", fileName);
        return success;
    }
...
void SeekMusicStream(Music music, float position)
{
    ...
    {
        TRACELOG(LOG_ERROR, "STREAM: Music stream buffer is NULL");
        return;
    }
